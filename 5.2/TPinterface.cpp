...
void TPinterface::initGUI()
{
	...
	addColumn();
	GLUI_Panel *robotTex= addPanel("Robot", 1);
	robotlistBox = addListboxToPanel(robotTex,"", NULL, 105);
	robotlistBox->add_item(1,"Referencia");
	robotlistBox->add_item(2,"Robot");
	
	GLUI_Panel *rbtdraw= addPanelToPanel(robotTex,"Desenho",106);
	addPanelToPanel(rbtdraw,"rbtdraw",106);
	GLUI_RadioGroup *robotGroup = addRadioGroupToPanel(rbtdraw,NULL,106);
	addRadioButtonToGroup(robotGroup, "Textured");
	addRadioButtonToGroup(robotGroup, "Wireframe");
...
void TPinterface::processGUI(GLUI_Control *ctrl)
{
	...
	switch (ctrl->user_id)
	{
		...
		case 106:
			{
				((LightingScene *) scene)->robot->changeTextured();
				break;
			}
		...
		case 105:
			{
				...
				((LightingScene *) scene)->robot->changeTexture(robotlistBox->get_int_val());
				break;
			}
